...
public abstract class DebugInfoBase {
    ...
    public void installDebugInfo(DebugInfoProvider debugInfoProvider) {
        ...
        debugInfoProvider.codeInfoProvider().forEach(debugCodeInfo -> debugCodeInfo.debugContext((debugContext) -> {
            ...
            debugCodeInfo.lineInfoProvider().forEach(debugLineInfo -> {
                ...
                Range subRange = new Range(symbolNameAtLine, stringTable, subMethodEntry, loAtLine, hiAtLine, line, primaryRange, isInlined, false, caller);
    ...
    private Range addCallersSubRanges(DebugInfoProvider.DebugLineInfo lineInfo, Range primaryRange, ClassEntry classEntry, DebugContext debugContext) {
        ...
        MethodEntry subMethodEntry = subClassEntry.getMethodEntry(lineInfo, this, debugContext);
        Range subRange = new Range(symbolName, stringTable, subMethodEntry, lo, hi, line, primaryRange, true, true, caller);
        classEntry.indexSubRange(subRange);
        try (DebugContext.Scope s = debugContext.scope("Subranges")) {
