...
public abstract class JSSEProviderSupport {
    ...
    public Provider getSunECProvider() {
        return SubstrateUtil.cast(new Target_sun_security_ec_SunEC(), Provider.class);
    }
}

@TargetClass(className = "sun.security.ec.SunEC", onlyWith = FipsEnabled.class)
@SuppressWarnings({"unused"})
final class Target_sun_security_ec_SunEC {

    @Alias
    @TargetElement(name = TargetElement.CONSTRUCTOR_NAME)
    native void originalConstructor();

    public Target_sun_security_ec_SunEC() {
        originalConstructor();
    }
}

@TargetClass(className = "sun.security.pkcs11.SunPKCS11", onlyWith = FipsEnabled.class)
@SuppressWarnings({"unused"})
...
@TargetClass(className = "sun.security.pkcs11.Token", onlyWith = FipsEnabled.class)
@SuppressWarnings({"unused"})
...
@TargetClass(className = "sun.security.pkcs11.wrapper.PKCS11", onlyWith = FipsEnabled.class)
...
final class NssConfigRetriever {
    ...
    static String getInlineConfig() {
        ...
        try (InputStream in = NssConfigRetriever.class.getResourceAsStream(file);
